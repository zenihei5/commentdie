...
static func apply_title_action_for_target(target: Node, action: String) -> Dictionary:
	...
	if action == "title_select" or action == "start":
		...
		open_codex = index == 2
		open_ranking = index == 3
		open_options = index == 4
...
static func front_state_action_for_target(target: Node, delta: float, title_action: String, result_action: String = "", ranking_action: String = "", options_action: String = "") -> Dictionary:
	...
	if state == "title":
		...
		if bool(title_result["openCodex"]):
			return {"handled": true, "action": "open_codex"}
...
static func has_modal_overlay(state: String) -> bool:
	return state in ["comment_choice", "gift_choice", "pause", "title", "ranking", "options", "character_select", "stream_frame_select", "collab_partner_select", "stream_start_intro", "boss_cutin", "tutorial", "result", "power_up_shop", "codex"]
...
static func shows_comment_countdown(state: String) -> bool:
	return not (state in ["title", "ranking", "options", "character_select", "stream_frame_select", "collab_partner_select", "stream_start_intro", "boss_cutin", "game_over_intro", "tutorial", "result", "pause", "power_up_shop", "codex"])
