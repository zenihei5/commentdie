...
func _ready() -> void:
	...
	_check(String(expert_relay_detail.get("bossText", "")).contains("EXPERTリレー") and String(expert_relay_detail.get("bossText", "")).contains("最終ボス撃破 未達成"), "expert relay shows reach and final boss information", failures)
