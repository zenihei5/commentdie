...
func _run_tests() -> void:
	...
	_check(String(special_model.get("kind", "")) == "specialBoss" and String(special_model.get("titleTag", "")) == "SPECIAL BOSS", "undiscovered special boss keeps only its safe title")
	...
	_check((special_model.get("hintLines", []) as Array).size() > 0 and not (special_model.get("conditionLines", []) as Array).is_empty(), "undiscovered enemy keeps broad hints")
